import { SAMPLE_VENUES } from "./sampleVenues";
import { CATEGORY_BY_ID, ADULT_CATEGORY_IDS } from "./categories";
import { COUNTRY_NAME } from "./countries";
import { store } from "./store";

const norm = (s) =>
  (s || "").toString().toLowerCase().normalize("NFKD").replace(/[\u0300-\u036f]/g, "");

export function isAdultCategory(categoryId) {
  return ADULT_CATEGORY_IDS.has(categoryId);
}

export function decorate(venue) {
  const cat = CATEGORY_BY_ID[venue.category];
  return {
    ...venue,
    country: COUNTRY_NAME[venue.countryCode] || venue.countryCode,
    categoryLabel: cat ? cat.label : venue.category,
    categoryIcon: cat ? cat.icon : "🍸",
    adult: venue.adult ?? Boolean(cat?.adult),
  };
}

function filterVenues(venues, { q, country, city, categories, includeAdult, minRating }) {
  const nq = norm(q);
  const ncity = norm(city);
  const wanted = categories && categories.length ? new Set(categories) : null;

  return venues.filter((venue) => {
    if (!includeAdult && venue.adult) return false;
    if (country && venue.countryCode !== country) return false;
    if (ncity && !norm(venue.city).includes(ncity)) return false;
    if (wanted && !wanted.has(venue.category)) return false;
    if (minRating && (venue.rating || 0) < minRating) return false;
    if (nq) {
      const haystack = norm(
        `${venue.name} ${venue.city} ${venue.country} ${venue.categoryLabel} ${venue.description}`
      );
      if (!haystack.includes(nq)) return false;
    }
    return true;
  });
}

export function sortVenues(venues, sort) {
  const copy = [...venues];
  if (sort === "name") copy.sort((a, b) => a.name.localeCompare(b.name));
  else copy.sort((a, b) => (b.rating || 0) - (a.rating || 0)); // default: rating
  return copy;
}

export function searchSampleVenues(params) {
  return sortVenues(filterVenues(SAMPLE_VENUES.map(decorate), params), params.sort);
}

// Community-submitted venues that have cleared moderation (see /admin).
export async function searchCommunityVenues(params) {
  const approved = await store.listApproved();
  return sortVenues(filterVenues(approved.map(decorate), params), params.sort);
}

// ---------------------------------------------------------------------------
// "Find me a better bar" — nearby alternatives to a venue the user is
// currently looking at, filtered by an optional vibe/category.
// ---------------------------------------------------------------------------

function haversineKm(lat1, lng1, lat2, lng2) {
  const R = 6371;
  const dLat = ((lat2 - lat1) * Math.PI) / 180;
  const dLng = ((lng2 - lng1) * Math.PI) / 180;
  const s =
    Math.sin(dLat / 2) ** 2 +
    Math.cos((lat1 * Math.PI) / 180) * Math.cos((lat2 * Math.PI) / 180) * Math.sin(dLng / 2) ** 2;
  return 2 * R * Math.asin(Math.sqrt(s));
}

function findNearbyVenues(venues, { lat, lng, excludeId, category, includeAdult, city, radiusKm = 5, limit = 8 }) {
  const ncity = norm(city);
  const pool = venues.filter((venue) => {
    if (excludeId && String(venue.id) === String(excludeId)) return false;
    if (!includeAdult && venue.adult) return false;
    if (category && venue.category !== category) return false;
    return true;
  });

  const hasCenter = lat != null && lng != null;
  const withDistance = pool.map((venue) => ({
    ...venue,
    distanceKm: hasCenter && venue.lat != null && venue.lng != null ? haversineKm(lat, lng, venue.lat, venue.lng) : null,
  }));

  // Prefer venues within the radius; widen to the same city, then to
  // whatever matches the vibe/adult filters, so sparse areas still get results.
  let nearby = hasCenter ? withDistance.filter((v) => v.distanceKm != null && v.distanceKm <= radiusKm) : [];
  if (!nearby.length && ncity) nearby = withDistance.filter((v) => norm(v.city).includes(ncity));
  if (!nearby.length) nearby = withDistance;

  return sortVenues(nearby, "rating").slice(0, limit);
}

export function nearbySampleVenues(params) {
  return findNearbyVenues(SAMPLE_VENUES.map(decorate), params);
}

export async function nearbyCommunityVenues(params) {
  const approved = await store.listApproved();
  return findNearbyVenues(approved.map(decorate), params);
}

// ---------------------------------------------------------------------------
// Live search — Google Places API (New) Text Search. Enabled when
// GOOGLE_PLACES_API_KEY is set. Covers every country with real venues.
// ---------------------------------------------------------------------------

const PLACES_ENDPOINT = "https://places.googleapis.com/v1/places:searchText";
const PLACES_FIELDS = [
  "places.id",
  "places.displayName",
  "places.formattedAddress",
  "places.location",
  "places.rating",
  "places.userRatingCount",
  "places.priceLevel",
  "places.websiteUri",
  "places.googleMapsUri",
  "places.internationalPhoneNumber",
  "places.currentOpeningHours.openNow",
].join(",");

const PRICE_LEVEL_MAP = {
  PRICE_LEVEL_INEXPENSIVE: 1,
  PRICE_LEVEL_MODERATE: 2,
  PRICE_LEVEL_EXPENSIVE: 3,
  PRICE_LEVEL_VERY_EXPENSIVE: 4,
};

function buildTextQuery({ q, country, city, categoryId, lat, lng }) {
  const cat = CATEGORY_BY_ID[categoryId];
  const parts = [q, cat ? cat.placesQuery : "bar"];
  if (lat != null && lng != null) {
    parts.push("near me");
  } else {
    const place = [city, country ? COUNTRY_NAME[country] : ""].filter(Boolean).join(", ");
    if (place) parts.push(`in ${place}`);
  }
  return parts.filter(Boolean).join(" ");
}

async function placesTextSearch(apiKey, params) {
  const body = { textQuery: buildTextQuery(params), pageSize: 20 };
  if (params.country) body.regionCode = params.country;
  if (params.minRating) body.minRating = params.minRating;
  if (params.lat != null && params.lng != null) {
    body.locationBias = {
      circle: { center: { latitude: params.lat, longitude: params.lng }, radius: (params.radiusKm || 5) * 1000 },
    };
  }

  const res = await fetch(PLACES_ENDPOINT, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      "X-Goog-Api-Key": apiKey,
      "X-Goog-FieldMask": PLACES_FIELDS,
    },
    body: JSON.stringify(body),
  });
  if (!res.ok) {
    const detail = await res.text().catch(() => "");
    throw new Error(`Places API ${res.status}: ${detail.slice(0, 300)}`);
  }
  const data = await res.json();
  return (data.places || []).map((p) => decorate({
    id: p.id,
    name: p.displayName?.text || "Unnamed venue",
    category: params.categoryId,
    city: params.city || "",
    countryCode: params.country || "",
    address: p.formattedAddress,
    lat: p.location?.latitude,
    lng: p.location?.longitude,
    rating: p.rating,
    ratingCount: p.userRatingCount,
    price: PRICE_LEVEL_MAP[p.priceLevel],
    website: p.websiteUri,
    googleMapsUri: p.googleMapsUri,
    phone: p.internationalPhoneNumber,
    openNow: p.currentOpeningHours?.openNow,
    demo: false,
  }));
}

export async function searchLiveVenues(apiKey, { q, country, city, categories, includeAdult, minRating, sort }) {
  let cats = categories && categories.length ? categories : ["bar"];
  if (!includeAdult) cats = cats.filter((c) => !ADULT_CATEGORY_IDS.has(c));
  if (!cats.length) return [];

  // One Places query per selected category, merged and deduped by place id.
  const batches = await Promise.all(
    cats.map((categoryId) =>
      placesTextSearch(apiKey, { q, country, city, categoryId, minRating }).catch((err) => {
        console.error(`Places search failed for category ${categoryId}:`, err.message);
        return [];
      })
    )
  );

  const seen = new Map();
  for (const venue of batches.flat()) {
    if (!seen.has(venue.id)) seen.set(venue.id, venue);
  }
  return sortVenues([...seen.values()], sort);
}

// Live nearby search for "find me a better bar" — same Places Text Search
// as above, but biased around a lat/lng instead of a city/country name.
export async function searchNearbyLiveVenues(apiKey, { lat, lng, excludeId, category, includeAdult, radiusKm = 5 }) {
  if (lat == null || lng == null) return [];
  let cats = category ? [category] : ["bar"];
  if (!includeAdult) cats = cats.filter((c) => !ADULT_CATEGORY_IDS.has(c));
  if (!cats.length) return [];

  const batches = await Promise.all(
    cats.map((categoryId) =>
      placesTextSearch(apiKey, { categoryId, lat, lng, radiusKm }).catch((err) => {
        console.error(`Nearby Places search failed for category ${categoryId}:`, err.message);
        return [];
      })
    )
  );

  const seen = new Map();
  for (const venue of batches.flat()) {
    if (excludeId && String(venue.id) === String(excludeId)) continue;
    if (!seen.has(venue.id)) seen.set(venue.id, venue);
  }
  return sortVenues([...seen.values()], "rating");
}
